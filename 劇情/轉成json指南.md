...
-   `entryID` (Number): 唯一的節點 ID。**新寫的檔從 1 起、沿陣列嚴格遞增、不留空號、不借位**——規則見下方〈⚠ 節點順序與編號〉（2026-09-17 作者裁示）。
-   `actorID` (String/Number): 說話者的 ID。
    ...
    -   `MC24`: 蔡邕
    -   `role105`: 茶博士
    -   `role38`: 張角（**報名之後**的說話者；報名前一律 `MC13` 張仲景。2026-09-23 作者給，〈張角現身〉）
    -   `role59`: 程寶（太平道；無立繪。2026-09-23 作者給）
    -   `role141`: 韓梁（太平道；無立繪。2026-09-23 作者給）
...
-   `Description` (String, 選填): **節點功能標籤**——給作者在 Unity 對話圖上一眼看出這格「做了什麼」。**只准寫下面詞表裡的詞，多項用頓號「、」並列，不寫任何解釋**（2026-09-17 作者裁示）。
    ...
    **詞表（2026-09-17 作者定，之後有多的再補）：**
    ...
    | `Conditions: IsCultureFit／IsBackGroundFit` 的條件格 | 文化分支：鮮卑／背景分支：世家子弟（§4.4） |
    | `BeginFight` | 進入戰鬥：〈對手〉 |
    | 戰鬥緩衝格 | 戰鬥緩衝節點 |
    ...
    **不准寫進這欄的：** 台詞內容摘要（「事後報告：她吃了」）、為什麼這樣寫、設定出處（`饕餮.md 第六節`）、紅線（「不得改成一個人」）、給作者的問題、好感／經驗的數值、選項的文字。**這些放創作稿的 `> ⚠` 引用區塊**（要作者做的事另記 `給AI看的指南/跨機待辦.md`），回讀稿的 `- 註記：` 照抄 `Description` 原文即可。**交檔前 `json順序檢查.py` 會逐條對詞表**，「是說明文」「不在詞表」列出來就要改——定樁：2026-09-18〈北邊起煙〉`主線事件/179年/11月.json` `#1`「史源＝《後漢書》…」、`#9`「門閂聲目前叫不到，待 Unity 新增…」都是寫錯欄位，那些話已經在創作稿檔頭和跨機待辦裡了。

## ⚠ 節點順序與編號（2026-09-17 作者裁示；只管之後新寫的 JSON）

**為什麼要管：** Bridge 建新對話時**沒有排序這回事**——它照 JSON 陣列由上到下發 Unity 流水號（START 是 0、陣列第一格是 1），畫布也照陣列順序排；`entryID` 只存進 `JsonEntryID` 印記，Bridge 不拿它排序。所以**陣列一亂，Unity 的 Inspector 編號和畫布左右順序就跟劇本對不上**。AI 新寫的檔常見兩種亂法：「分支借位」（從 `#42` 岔出去就編 `#421`、`#521`、`#685`，仗義疏財）和「後補的格子塞在陣列中間」（`#15` 後面直接接 `#4335`，9月；`#7` 之後跳 `#15`、`#20` 再回 `#8`，五原障）。Unity 匯出的檔天然整齊（小溪村那批），拿它當標準。

**規則（新寫 JSON 一律遵守）：**

1. **陣列順序就是劇本閱讀順序。** 主線一路往下；遇到選項，各選項格**連號排在一起**，接著**先寫完第一個選項的整段分支，再寫第二個**；成功／失敗分支同理，先寫完一條再寫下一條。
2. **`entryID` 從 1 起、單純遞增、不借位、不留空號、不用大號段標分支。** 陣列第 N 格就是 `#N`。
3. **匯合點和往回接的格子**照它在劇本中**第一次出現**的位置編號，後面的分支用 `links` 指回去就好，不另開一格。
4. **事後補格子**（檔已進 Unity 之後再加）：號碼接在該對話**目前最大號之後**，陣列位置放在它劇情上該在的地方；新格沒有印記，Bridge 會建新格、不動舊格（見 `給AI看的指南/Unity對話同步流程.md` 鐵則）。
5. **一旦同步進 Unity 就不能重新編號。** 印記、總覽稿的 `#entryID` 錨點、跨對話連線都認這個號，改號等於砍掉重匯。**已完成並匯入的檔（五原障、仗義疏財、9月、死地生綠……）一律維持現狀，不回頭整理。**

**交檔前跑一次：** `python 給AI看的指南/json順序檢查.py <檔>`——重號、斷鏈、首格非 1、號碼不遞增、留空號是硬錯，列出來就要改；「陣列順序 ≠ 照 links 走」只是提示（作者舊檔在條件路由格有時一層一層排，兩種都讀得順）。**舊檔跑出來的警告不要去改。**
...
-   **觸發條件**: 創作稿的 `Sequence` 標了就轉，在 `SetPortrait` 後緊跟 `EnableCharacterExpression`。**⚠ 創作稿一整場一個都沒標，是創作稿漏了**（文本創作指南 2.3：一場戲不得零表情，2026-09-16 作者指正）——轉檔前先回創作稿補，不是照轉成零。
...
-   ⚠ **`LoadLevel` 例外：不掛在 `［換景指令］@1` 槽裡，另開一格單獨放，下一句一定是 `Continue()`**——`LoadLevel(X);Continue();`，轉場四段留在前一格。見 §2.6 與本檔〈轉檔前必掃〉。
...
> ### ⚠⚠ 通則：選項一律是「短版選單句 ＋ 底下那格完整發言」兩段（作者 2026-09-05 裁示，**不分有無檢定**）

> **⛔ 選單前的那一格不掛表情特效（2026-09-23 作者裁示）。** 選項節點本身沒有 `Sequence`，關不掉上一格開的特效——玩家在選的時候，那個問號／燈泡／星星就一直掛在選單旁邊（作者附遊戲截圖：「該怎麼做呢？」選單右側懸著一顆 `Question`）。所以**`links` 指到選項的那一格，`Sequence` 裡不得有 `EnableCharacterExpression`**；要表情就往前挪一格、在選單前一格開頭關掉。主角立繪照 7.2 #4 切 `pic=12` 即可，**不配 `Meditate`**。特效可以掛在選完之後的發言格（§4.2 通則的那格），它有下一格可以關。 `json順序檢查.py` 會抓「選單前一格掛特效」。
>
> **結構（兩種都一樣，只差中間有沒有擲骰格）：**
> - **無檢定：`選項節點` → `主角發言節點`（`actorID: MC1`，帶立繪）→ 分支**
> - **有檢定：`選項節點` → `空對話擲骰節點` →（既有的空緩衝格）→ **`主角發言節點`**（`actorID: MC1`，帶立繪）→ 成敗分支**
>
> > **⚠ 為什麼發言格擺在擲骰之後（作者 2026-09-05 裁示）：擲骰格之後一定要有一個過渡格子，然後才是分支**——而**發言格正好可以當那個過渡格子**。也就是說那個位置是**同一個「過渡槽」，空緩衝格與發言格擇一即可**：
> > - **新寫**：`選項 → 擲骰格 → 發言格 → 成敗分支`，發言格就是過渡格，不必再多放一個空的。
> > - **既有檔已經有空緩衝格**：不必刪，發言格接在緩衝格後面、分支前面即可（本批 7 處這樣接）。
> >
> > **⚠ 過渡格由誰擔任都可以（作者 2026-09-05 補充）：空緩衝格、主角發言格、NPC／旁白台詞都算。** 重點只有一個——**擲骰格與成敗分支之間一定要隔一格**。全案 188 條擲骰流的下一格實況（同日掃描）：空緩衝格 **124**、主角發言 **62**、NPC 台詞 **2**。
> >
> > 兩種都跑得動（掃描 98 條既有檢定發言流）：`擲骰→發言` **62 處**、`擲骰→空緩衝→發言` **33 處**、`擲骰→空緩衝→空緩衝→發言` **3 處**。**`IsPassDice()` 的判定掛在分支節點上，中間隔幾個過渡格都不影響。**
> >
> > **⚠ 兩個選項共用同一個擲骰格是可以的**（作者同日確認，理由是後面仍有成敗分支）。**代價要知道：匯流之後只會有一格發言，兩個選項共用同一句**，所以那一句要寫成兩邊都接得上的話。實例：`趙王洞/遺跡.json` `#4342`／`#4343` 共用擲骰格 `#4344`，發言格 `#4402` 寫成「這東西還在我身上。妳倒是給句話。」——不論玩家選的是「會出事嗎？」還是「我拿著才安穩」，都接得下去，也接得住成敗兩條旁白（「她看了你一眼」／「她還是沒有答」）。
> >
> > ⚠ 全案兩種寫法本來都有（2026-09-05 掃描：擲骰在發言前 92 處、發言在擲骰前 118 處）。**既有的不必回頭改，新寫一律照本節。**
>
> **為什麼一定要有那一格：選項是選單，不是台詞本體。** 玩家真正在對話框裡看見主角把話說出口的，是下面那一格；少了它，對方等於在回應一句沒說完的話。**2026-09-05 獨立重掃（覆核首次盤點的 775／278）：全案 956 個選單選項，套下方豁免後缺發言格 142 處**——漏成這樣，就是因為本節的規矩以前只寫在「有檢定」那條流程底下。**✅ 同日全部補完**：作者逐條核可後補上 96 個新發言格（30 份 JSON）＋填 1 個既有空節點，另 29 處「選項與發言格逐字相同」把發言格寫長；未補的是 26 處〔非主角〕與 21 處〔豁免〕。（完工紀錄檔 （原完工紀錄檔已於 2026-09-07 刪除，查 git 歷史） 已於 2026-09-07 作者確認完工後刪除；需回溯查 git 歷史）。
>
> **選項文字怎麼寫：**
> 1. **一定要比底下那句短。** 它是選單，不是台詞本體。
> 2. **內容是同一件事**——可以是完整發言的濃縮版、前半句，也可以是**同一個意思換句話說**；重點是玩家看了就知道「選下去我等一下會講出類似的話」。
> 3. **必須是他第一人稱說出口的話，不是動作或意圖的描述。**
>    - ⛔ `「反駁娜娜說的是錯的」`（這是在描述他要幹嘛，不是他的話）
>    - ✅ `「妳這說法有問題。」` → 底下那格：`「妳這說法有問題——妳說天機盤指著後山，可我們昨天才從後山回來。」`
> 4. **短得無可再縮的選項（「走吧。」「赤龍幫。」這類短答），或發現選項與發言格逐字相同——把發言格寫長，選項不動**（2026-09-05 作者裁示；全案 29 處已同日照此改）。
> 5. 四軸／檢定的後台名稱不進選項文字（檢定選項的 `[em2][XX檢定][/em2]` 前綴是引擎標記，不算）。
>
> **豁免（不接發言節點，2026-09-05 作者裁示）：**
> - **不開口選項**：括號動作（`（什麼都不說，自己頂上去）`，見 `給AI看的指南/武俠文風創作指南.md` 7.2）。
> - **探察／查看選單**：選項是名詞或地點（「灶」「架上的簡牘」「牆上的刻字」）——那是「看哪裡」，不是「說什麼」，選完直接接旁白。
> - **樞紐選單的功能鈕**：「離開」「再逛逛」「準備好了」這類，選完由引擎收場、回選單或進戰鬥。
> - **結構特例**：`根據地/小表妹` 的閒聊選單——選項本身就是小犀的台詞，不套本通則。
>
> 口吻準則見 `給AI看的指南/武俠文風創作指南.md` §四〈你（主角）〉與 7.3，**兩邊講的是同一條規則。**
...
-   **關鍵規則**: 當玩家需要從多個選項中選擇，並且某些選項涉及屬性/專長檢定時，處理流程如下：
    ...
    3.  **選項節點**: **每個玩家選項本身都是一個獨立的JSON節點**，擁有自己的 `entryID`。
        -   `actorID`: **一律 `MC1`（已定，2026-09-08 作者確認；本行原寫「通常為 `0`…取決於遊戲如何呈現選項發起者」，是錯的）。** 選項是主角要說的話，發起者就是主角；寫 `0` 的話 Unity 對話節點的 **Actor 欄會是空的**。
            > ⚠ **這條早在 `給AI看的指南/擲骰指令轉換規則.md` 就標過「***重要更新：選項發起者改為 MC1（主角）***」**（該檔 §範例 `#102`），只有本節沒跟上，於是新寫的檔案照本節寫成了 `0`。**兩份指南以「一律 `MC1`」為準。**
...
### 4.4 背景／文化條件分支（`[em3]` 標籤；2026-09-18 作者以〈北邊起煙〉定樁）

主角的**出身背景**與**文化**是開檔就定的屬性，可以拿來開一條「只有這種人才說得出口」的台詞或旁白。寫法三件事：標籤、條件、排列。

**1. 標籤：`[em3][類別：值][/em3]`，全形冒號，寫在那一格文字最前面**（發言格放在「」裡面的開頭，旁白格放在 `[panel=6]` 之後）。全案既有的背景分支一律是這個形（`[em3][背景：世家子弟][/em3]`、`[em3][背景：流民][/em3]`），文化照同一個模子：

```
「[em3][文化：鮮卑][/em3]他們年年這時候來，不是挑日子。草枯了，牲口撐不到開春，南邊有糧——他們不是來打仗的，是來過冬的。」
```

> ⚠ 不寫 `[鮮卑文化]`、不寫半形冒號 `[文化:鮮卑]`——玩家眼裡「背景：」「文化：」「檢定」這幾種標籤要長得像一家人（`[em2][口才檢定][/em2]` 也是「類別」在前）。作者 2026-09-18 定：**`[文化：鮮卑]` 這個形**。`Json/主線事件/179年/11月.json` `#16` 目前是半形冒號，作者在 Unity 端改。

**2. 條件寫在 `Conditions`，結尾一定有分號：**

```
IsCultureFit("Player", "Xianbei") == true;
IsBackGroundFit("Player", "Noble") == true;
```
...
| 背景 | 街頭混混 | `Charlatan` |
| 背景 | 遊牧民族 | `Outlander` |
| 背景 | 流民 | `Beggar`（`破廟/阿傑…` `#3216` 在用。**顯示名一律「流民」，作者 2026-09-18 定**——原本寫「遊民」的兩處已同日改齊：`小溪村/牧羊人、茶博士.json` `#2052`、`探索事件/179年/9月.json` `#3907`。⚠ 這兩格**仍沒掛 `Conditions`**，照現況不分背景都會播，待作者補） |

文化 ID 是作者 2026-09-18 給的三個；表上沒有的值**不得自創**，先問。

> ✅ **`破廟/阿傑、阿佑、阿偉、水井.json` 的五條背景分支已於 2026-09-18 依作者交辦改成本節的形**（`#3214` `HumbleHero`／`#3215` `Charlatan`／`#3216` `Beggar`／`#3220` `Noble`／`#3221` `Outlander`）：`#3221` 的條件由 `IsCultureFit("Player ", "Outlander")`（拿背景 ID 當文化、且參數尾隨一個半形空白，條件永遠不成立）改為 `IsBackGroundFit("Player", "Outlander")`；五格標籤由舊式 `[em7]［遊牧民族］[/em7]「…」` 換成 `「[em3][背景：遊牧民族][/em3]…」`。**這一份現在可以照抄。**

**3. 排列：條件格在前、預設格在後，兩條各自走到同一個匯合點。** 上一格的 `links` 先列有條件的格，再列沒有 `Conditions` 的預設格（`#5` → `[16, 6]`：`#16` 鮮卑文化版直接接 `#9`，`#6` 一般版走 `#7`、`#8` 再到 `#9`）。**預設格不掛條件**，這樣不符合任何文化的玩家一定有路走。條件格是一格就一格，不必湊成兩格。

**4. `Description`：** 條件格寫「文化分支：鮮卑」／「背景分支：世家子弟」（§1 詞表）。
...
-   **玩家選項情境**:
    ...
    -   **每一個選項節點** (`actorID` **一律 `MC1`**, `text: "選項一的文字"`) 的 `links` 則指向其對應的**空對話擲骰節點** (如果需要檢定)，或**直接指向該選項的「主角發言節點」** (如果無檢定)。**兩種情形，分支的第一句話都必須是主角把那句話說完**——見 §4.2 開頭 ⚠⚠ 通則。
...
## ⚠ 轉檔前必掃：台詞可見字數 50 內、紅線 60（2026-09-04 作者定）

**每一格台詞（非旁白）剝掉 `[panel]`／`[em2]`／`[em7]`／`[var]` 標籤後，50 字以內；過 60 是紅線，必拆格。** `[em7]…[/em7]` 中間的動作文字有顯示、要算。規則全文見 `給AI看的指南/文本創作指南.md` 2.1b；掃描用 `python 給AI看的指南/文風節奏檢查.py <檔>`（過 50 列出、過 60 標紅線）。定樁：`大地圖/幽冥禁地.json` `#2`／`#4` 與 `妖林.json` `#4`（原 66／98／88 字，2026-09-04 各拆兩格）。

## ⚠ 轉檔前必掃：對話開頭不得有空白（2026-09-01 新增）

**每一格的 `text`（連同 `zh_TW`／`zh_CN`／`en`）第一個字元不可以是空白。** 開頭一個半形空格，遊戲內排版就會多吃一次換行，整格對話往下掉一行。

- ✅ `"text": "[panel=2]「哇啊，別再說啦！」"`
- ❌ `"text": " [panel=2]「哇啊，別再說啦！」"`（開頭多一個空格）

句中的空白不管（`[em2][威嚇檢定][/em2] 「你以為……」` 這種標籤後的空格是刻意的），**只掃開頭**。2026-09-01 清過一批：`破廟/開場、蕭靈犀.json`、`破廟/阿傑、阿佑、阿偉、水井.json` 共 20 個欄位。

## ⚠ 轉檔前必掃：`LoadLevel` 下一句一定是 `Continue()`（2026-09-01 作者拍板）

**`LoadLevel(...)` 之後同一格剩下的指令不保證跑得到**（場景已經換掉了）。所以：

- **`LoadLevel` 自己占一格**：`"Sequence": "LoadLevel(Command_YunzhongInn);Continue();"`，`actorID "0"`、`text ""`。
- **不掛 `@`**，也不要塞進轉場的 `［換景指令］@1` 槽——轉場四段留在**前一格**，`LoadLevel` 接在它後面自成一格。
- **要收的東西（`MapLock`、`ModifyData`、`DisableEventBG`……）全部放在 `LoadLevel` 之前**，不要放後面。
- 唯一例外：整格是等訊號的（`Continue()@Message(EndVideo)`），`LoadLevel` 跟著掛同一個訊號、排在 `Continue()` 前一句。

2026-09-01 清過一批 6 處（`主線事件/179年/7月` `#2052`／`#2341`、`趙王洞/山賊窩` `#4163`、`趙王洞/遺跡` `#4237`、`大地圖/雲中/街道` `#114`、`呂信線/五原障` `#70`）。**`AndyCursor用/DialogueJsonBridge.cs` 匯入時會擋、匯出時會在 Console 點名**，但創作稿階段就該寫對。

---

## 注意事項

-   **節點順序與編號（必守，2026-09-17）**：新寫的 JSON 陣列順序＝劇本閱讀順序，`entryID` 從 1 起連號遞增、不借位、不留空號、分支不用大號段；補格用最大號＋1、放在劇情該在的位置；進了 Unity 就不改號。交檔前跑 `json順序檢查.py`。詳見上方〈⚠ 節點順序與編號〉。
...
-   **⛔ `SetFlag`／`GetFlag` 不是引擎指令，一律禁用（已定，2026-08-27 作者指出）。** 對話**不能有旗標**。要記住「玩家做過什麼」只有兩條合法途徑：
    ...
    -   **⛔ 任務代號同理（2026-09-02 作者重申）：`CF` 編號、任務與 entry 都由作者在 Unity 建**——AI 不得自取編號、也不得「暫記」一個號；創作稿一律寫 `＿＿` 留白，等作者填。閘門要掛哪個主線任務狀態，同樣由作者定。
    ...
    -   **能用純樹狀分岔解決的，不要記狀態。** 子羽線六支已於 2026-08-25 拍板改為零旗標（見 `@角色設定/子羽.md`、`劇情/子羽/04 歸姓.md`）。
...
**轉換後 JSON (片段示意，重點演示選項處理 - 最新修訂流程):**（⚠ 依 §4.2 開頭通則：選項節點放**短版選單句**——就算原始劇本的選項寫的是整句，轉換時也要濃縮；完整發言在下一格）
```json
[
  {
    "entryID": 100,
