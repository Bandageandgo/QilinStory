...
>
> **結構（兩種都一樣，只差發言格之後有沒有擲骰格）：**
> - **無檢定：`選項節點` → `主角發言節點`（`actorID: MC1`，帶立繪）→ 分支**
> - **有檢定：`選項節點` → `主角發言節點`（`actorID: MC1`，帶立繪）→ `空對話擲骰節點`（→ 空緩衝格）→ 成敗分支**
>
> > **⚠ 發言格一律緊接在選項後面（2026-09-05 施工時定案，本行原寫「擲骰格在發言格之前」，已改）。** 三個理由：
> > 1. **共用擲骰格的場合只有這一種寫法成立**——`趙王洞/遺跡.json` 兩個選項（`#4342`／`#4343`）指向同一個擲骰格 `#4344`；發言格若擺在擲骰之後，兩條選項會共用同一句台詞，選了哪一句就分不出來。
> > 2. **全案兩種寫法本來都有**（2026-09-05 掃描：發言在擲骰前 118 處、擲骰在發言前 92 處），沒有既定慣例可循。
> > 3. **與作者親手重建的 `呂信線/02第三百四十九次.json` 同形**（`#32` 選項 → `#40` 發言 → `#42` 擲骰 → `#43` 緩衝 → 分支）。
> >
> > 語意上也順：**先把話說出口，再擲骰看這一手成不成**。既有那 92 處相反寫法**不必回頭改**，新寫一律照本節。
>
